...
Tests AST-based structural style checks (preferences.preferences). The preferences API is a registry of single-node
 `Check` functions (each takes one `ast.AST` node and returns a complaint string or None), plus
`preferences_violations`, which walks a file once and returns a dict grouping every complaint by check kind. These
tests exercise that API with real code that trips — and real code that must not trip — each check.
...
# preferences.py are optional AST checks. Repo-owner can keep or delete.
preferences_violations: Callable[[str, str], str] | None
try:
    preferences = importlib.import_module("preferences.preferences")
except ImportError:
    preferences = None
...
CHECKS = preferences.CHECKS if preferences else {}


@strategies.composite
def identifiers(draw: strategies.DrawFn) -> str:
    """Draw a valid ASCII Python identifier that is not a keyword."""
    first = draw(strategies.sampled_from(string.ascii_letters + "_"))
    rest = draw(strategies.text(alphabet=string.ascii_letters + "_" + string.digits, max_size=24))
    name = first + rest
